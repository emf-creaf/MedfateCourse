...
3. The `growth()` model implements several **daily** processes:
   ...
   ii) Structural changes (diameter and height for trees; cover and height for shrubs) derived from growth and senescence processes.
   iii) Tree mortality, which reduces cohort density.
...
[^1]: Hölttä et al (2017). Tree physiology, 37, 851–868.
...
* $\Delta LA_{sink}$, $\Delta SA_{sink}$ and $\Delta B_{fineroot,sink}$ are the increments expected due to **growth rates**, which are affected by temperature and turgor limitations [^1]. 
:::
::::
...
[^1]: Cabon et al (2020).  New Phytologist, 226, 1325–1340.
