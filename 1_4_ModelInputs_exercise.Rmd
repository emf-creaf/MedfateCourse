...
## Mapping trees from the control stand (1)
...
## Mapping trees from the control stand (2)
...
## Mapping trees from the control stand (3)
...
and use this information to map the tree data again, where we specify the parameter `plot.size`:
...
We run again the summary and check whether basal area and leaf area index make sense:
...
## Mapping trees from the control stand (4)
...
We should somehow estimate tree heights, for example using an allometric relationship:
...
and rerun the tree data mapping function.
...
## Mapping trees from the managed stand (1)
...
## Mapping trees from the managed stand (2)
...
## Mapping trees from the managed stand (3)

Now we repeat our mapping and check the results:

.code80[
```{r, eval = TRUE, highlight.output=c(2,5)}
pobl_thi_bef$treeData <- forest_mapTreeTable(subset(poblet_trees,Plot.Code=="POBL_THI_BEF"), 
                                             mapping = mapping, SpParams = SpParamsMED, 
                                             plot.size = sampled_area)
...
## Mapping trees from the managed stand (4)
...
## Reducing the number of cohorts (1)
...
## Reducing the number of cohorts (2)
...
## Reducing the number of cohorts (3)
...
We can inspect the soil definition using `print()`
