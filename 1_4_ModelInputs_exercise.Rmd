...
Once tree heights are defined, we can include them in our mapping:
...
```{r, eval = FALSE}
pobl_thi_bef$treeData <- forest_mapTreeTable(poblet_trees[poblet_trees$Plot.Code=="POBL_THI_BEF",], 
                                             mapping = mapping, SpParams = SpParamsMED, 
                                             plot.size = sampled_area)
summary(pobl_thi_bef, SpParamsMED)
```

Like the control plot, the `summary()` indicates a dense oak forest. We can finally map tree data for the forest plot after the thinning intervention:

```{r, eval = FALSE}
pobl_thi_aft = emptyforest("POBL_THI_AFT")
pobl_thi_aft$treeData <- forest_mapTreeTable(poblet_trees[poblet_trees$Plot.Code=="POBL_THI_AFT",], 
                                mapping = mapping, SpParams = SpParamsMED, 
                                plot.size = sampled_area)
