...
```{r}
fb <- readRDS("exercises/StudentRdata/fb_data.rds")
names(fb)
```
...
```{r}
fb_control_buckets <- defaultControl(transpirationMode = "Granier", soilDomains = "buckets")
fb_control_single <- defaultControl(transpirationMode = "Granier", soilDomains = "single")
fb_control_dual <- defaultControl(transpirationMode = "Granier", soilDomains = "dual")
```

and we assemble our inputs into a `spwbInput` object, using:

```{r}
x_buckets <- spwbInput(fb_forest, fb_soil, fb_SpParams, fb_control_buckets)
x_single <- spwbInput(fb_forest, fb_soil, fb_SpParams, fb_control_single)
x_dual <- spwbInput(fb_forest, fb_soil, fb_SpParams, fb_control_dual)
```
...
```{r, eval=TRUE}
fb_basic_buckets <- spwb(x_buckets, fb_meteo,  elevation = 420, latitude = 43.24083)
fb_basic_single <- spwb(x_single, fb_meteo,  elevation = 420, latitude = 43.24083)
fb_basic_dual <- spwb(x_dual, fb_meteo,  elevation = 420, latitude = 43.24083)
```
