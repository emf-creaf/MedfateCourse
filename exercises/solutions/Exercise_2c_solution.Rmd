...
The error indicate that the rosemery scientific name used (Rosmarinus officinalis) is not found in SpParams. Thus, after check SpParams and replace it with its accepted name (Salvia rosmarinus) we can now initialize the water balance input
...
```{r, eval=TRUE}
tv_spwb1 <- spwb(tv_x1, tv_meteo, elevation = 555, latitude = 41.053)
```
...
g1 <- plot(tv_spwb1, type = "LFMC", bySpecies = T) +
...
```{r}
tv_modeled <- as.data.frame(summary(tv_spwb1, freq="days",FUN=mean, output="LFMC", bySpecies = T))
...
**Interpretation**: We can observe that Salvia rosmarinus LFMC simulation performed a better agreement between modeled and field observed data than Quercus coccifera simulation. 
...
```{r}
FireHazard<-as.data.frame(summary(tv_spwb1, freq="days",FUN=mean, output="FireHazard"))
...
#g1
#Fire plot
g2<-ggplot(FireHazard, aes(x=dates, y=I_b_surface, colour = "darkred"))+
  scale_x_date(date_breaks = "1 month", date_labels = "%m")+
  geom_line()+
  ylab("Fireline intensity (kW/m)")+
  theme_bw()+
  theme(legend.position = "none")
#Arrange  
